...
for dir in $PWD/*Node* ; do
  if [[ -d "$dir" ]]; then
    ...
    if [[ $node_name == "Node-1" ]]; then
        ...
        besu --data-path=data --miner-enabled --miner-coinbase=0x4b92b29b10bf4c802f7a3471c2b27c8afbb5d507 --genesis-file=../genesis.json --rpc-http-enabled --rpc-http-api=ETH,NET,IBFT,EEA,PRIV --host-allowlist="*" --rpc-http-cors-origins="all" --privacy-enabled --privacy-url=http://127.0.0.1:9102 --privacy-public-key-file=Tessera/nodeKey.pub --min-gas-price=0 &
    elif [[ $node_name == "Node-2" ]]; then
        echo "You entered 'Node-2'"
        besu --data-path=data --miner-enabled --miner-coinbase=0x5e9d71d8a6c4aff4aefb0c8d9d9f719f89d85f74 --genesis-file=../genesis.json --bootnodes=$enodeURL --p2p-port=30304 --rpc-http-enabled --rpc-http-api=ETH,NET,IBFT,EEA,PRIV --host-allowlist="*" --rpc-http-cors-origins="all" --rpc-http-port=8546 --privacy-enabled --privacy-url=http://127.0.0.1:9202 --privacy-public-key-file=Tessera/nodeKey.pub --min-gas-price=0 &
    elif [[ $node_name == "Node-3" ]]; then
        echo "You entered 'Node-3'"
        besu --data-path=data --miner-enabled --miner-coinbase=0x2f4ba0e86d3e9b9fdd7c5a64e7f426a4144ea70f --genesis-file=../genesis.json --bootnodes=$enodeURL --p2p-port=30305 --rpc-http-enabled --rpc-http-api=ETH,NET,IBFT,EEA,PRIV --host-allowlist="*" --rpc-http-cors-origins="all" --rpc-http-port=8547 --privacy-enabled --privacy-url=http://127.0.0.1:9302 --privacy-public-key-file=Tessera/nodeKey.pub --min-gas-price=0 &
    else
        echo "You entered 'Node-4'"
        besu --data-path=data --miner-enabled --miner-coinbase=0x968c19c55cf8e5aeffd8c9f2aa6b13e7f7eef776 --genesis-file=../genesis.json --bootnodes=$enodeURL --p2p-port=30306 --rpc-http-enabled --rpc-http-api=ETH,NET,IBFT,EEA,PRIV --host-allowlist="*" --rpc-http-cors-origins="all" --rpc-http-port=8548 --privacy-enabled --privacy-url=http://127.0.0.1:9402 --privacy-public-key-file=Tessera/nodeKey.pub --min-gas-price=0
    fi
  fi
done